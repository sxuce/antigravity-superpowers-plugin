...
If you are unable to write these files directly, use `python .agent/skills/workflow/scripts/write_artifact.py --path <target>` to persist the content.
...
- `review`
- `finish`
- `rest-automation` (if relevant)
- `python-automation` (if Python)
...
3) If verification fails:
   - Stop.
   - Switch to systematic debugging (use `debug`).
