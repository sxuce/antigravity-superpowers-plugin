#!/usr/bin/env python3
"""Spawn an isolated Antigravity CLI subagent for focused task execution.

This enables parallel execution by launching independent agy instances
...
def find_repo_root(start: Path) -> Path:
    """Traverse upwards to find the repository root (containing .agent/ or .git/)."""
    curr = start.resolve()
    for _ in range(10):
        if (curr / ".agent").exists() or (curr / ".git").exists():
            return curr
        if curr.parent == curr:
            break
        curr = curr.parent
...
def spawn_subagent(
    ...
    skill_file = skills_dir / f"superpowers-{skill}" / "SKILL.md"
    if not skill_file.exists():
        skill_file = repo_root / f".agent/skills/superpowers-{skill}/SKILL.md"
    if not skill_file.exists():
        skill_file = repo_root / f"skills/superpowers-{skill}/SKILL.md"
...
"""
    ...
    cmd_list = ["agy"]
    if yolo:
        cmd_list.append("--dangerously-skip-permissions")
    cmd = " ".join(cmd_list)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Spawn an Antigravity CLI subagent for parallel execution"
    )
    parser.add_argument(
        "--skill",
        required=True,
